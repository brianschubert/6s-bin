...
class Helpers:
    ...
    def assert_embedded_isclose(s1: str, s2: str, **kwargs: Any) -> None:
        """
        Asserts than all embedded numbers between the two given strings are close in value.

        Does not consider the formatting of the two strings outside the number of
        embedded numbers they contain. So, for example, this function considers the strings
        "1 2" and "foo1 and2bar" to match.
        """
        if s1 == s2:
            return
